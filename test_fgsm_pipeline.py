...
def _prepare_face_tensor(face_crop_rgb: np.ndarray) -> torch.Tensor:
    """Convert an RGB face crop to a normalised FaceNet-ready tensor.

    Resizes the crop to 160 × 160 (FaceNet native resolution), converts
    to float32, normalises to [0.0, 1.0], and adds a batch dimension.

    Args:
        face_crop_rgb: RGB uint8 NumPy array of shape (h, w, 3).

    Returns:
        Float32 tensor of shape (1, 3, 160, 160) with values in [0.0, 1.0].
    """
    logger.info(
        "STEP 3 — Preparing face tensor. Resizing from %s to (%d, %d)…",
        face_crop_rgb.shape[:2],
        FACENET_INPUT_SIZE,
        FACENET_INPUT_SIZE,
    )

    # Resize to FaceNet input size using Lanczos interpolation (high quality).
    ...
    face_array = np.array(pil_resized, dtype=np.float32) / 255.0  # → [0.0, 1.0]

    # Convert HWC → CHW and add batch dimension: (H, W, 3) → (1, 3, H, W)
    face_tensor = torch.from_numpy(face_array).permute(2, 0, 1).unsqueeze(0)

    logger.info(
        "STEP 3 — Complete. Tensor shape: %s | Range: [%.4f, %.4f]",
...
def _tensor_to_uint8_rgb(tensor: torch.Tensor) -> np.ndarray:
    ...
    Args:
        tensor: Float tensor of shape (1, 3, H, W) with values in [0.0, 1.0].

    Returns:
        uint8 NumPy array of shape (H, W, 3) in RGB colour space.
    """
    # Remove batch dim, permute CHW → HWC, scale to [0, 255], cast to uint8.
    array = (
...
def main() -> None:
    ...
    adversarial_face_rgb = _tensor_to_uint8_rgb(adversarial_tensor)
    logger.info(
