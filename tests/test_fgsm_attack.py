...
test_fgsm_attack.py
-------------------
Unit tests for the FGSM adversarial attack module.
...
Run from project root:
    python -m pytest tests/test_fgsm_attack.py -v
"""
...
from src.ml_core.attacks.fgsm_attack import FGSMAttack  # noqa: E402
...
def attack_instance() -> FGSMAttack:
    """Instantiate a single FGSMAttack for the entire test module.

    Using ``scope='module'`` means the FaceNet model is loaded only once,
    significantly reducing test suite runtime.

    Returns:
        A ready-to-use ``FGSMAttack`` instance on CPU.
    """
    return FGSMAttack(epsilon=EPSILON, device="cpu")
...
class TestPerturbationGeneration:
    """Verify that the FGSM perturbation is actually applied."""
    ...
    def test_embedding_distance_increases_after_attack(
        ...
        The FGSM loss maximises cosine distance.  After a successful attack,
...
class TestCPUExecution:
    ...
    def test_full_attack_completes_on_cpu(
        ...
        try:
            ...
            pytest.fail(f"FGSM attack raised an exception on CPU: {exc}")
...
class TestReproducibility:
    ...
    def test_attack_is_deterministic_on_cpu(
        ...
        assert torch.allclose(adv1, adv2, atol=1e-6), (
            "FGSM attack output is not reproducible given the same seed and input."
        )
